/*
  ...
  $Id: fonts.c,v 1.21 2009/06/06 18:22:00 wkendrick Exp $
*/
...
TuxPaint_Font *load_locale_font(TuxPaint_Font * fallback, int size)
{
  ...
  if (need_own_font)
  {
    ...
    if (ret == NULL)
    {
//      snprintf(str, sizeof(str), "%s/%s.ttf", macosx.fontsPath, lang_prefix); dailin
      ret = TuxPaint_Font_OpenFont("", str, size);
    }	 
#endif
...
//  loadfonts_locale_filter(screen, DATA_PREFIX "fonts", locale);dailin
    loadfonts_locale_filter(screen, "fonts", locale);
...
//    loadfonts(screen, macosx.fontsPath);dailin
    loadfonts(screen, "/usr/share/fonts");
    loadfonts(screen, "/usr/X11R6/lib/X11/fonts");
